...
class Library(tk.Tk):
    ...
    def register(self):
        ...
        check = self.access.get_accounts()
        for row in check:
            if self.username_register_entry.get() == row[0]:
                coverframe = tk.Frame(self.register_frame, bg='blue', height=25)
                coverframe.place(relx=0.5, anchor='n', rely=0.85, relwidth=1)
                messages = tk.Message(self.register_frame, width=1000, font=('courier', 15),text="That username is already taken please choose another")
                messages.place(relx=0.5, rely=0.85, anchor='n')
                key = True
        ...
        if key == False:
            ...
            account = database()
            account.register_database(self.username_register_entry.get(), self.password_register_entry.get(),
                                              self.name_register_entry.get(), self.lastname_register_entry.get(),
                                              self.email_register_entry.get(), self.gender_register_entry.get(),
                                              self.age_register_entry.get())
    ...
    def editprofile_page(self):
        ...
        #User is able to input their new information
